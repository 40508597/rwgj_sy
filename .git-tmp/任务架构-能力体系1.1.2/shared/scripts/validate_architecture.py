...
ENTRY_KEYS = ["用户入口", "接口入口", "事件入口", "系统入口"]
ENTRY_OPTIONAL_KEYS = ["命令入口", "资源入口"]
...
def validate_architecture(data: dict[str, Any], root: Path, stage: str = "full") -> tuple[list[str], list[str]]:
    ...
    required_keys = REQUIRED_TOP_KEYS if stage == "full" else [k for k in REQUIRED_TOP_KEYS if k not in SKELETON_OPTIONAL_KEYS]

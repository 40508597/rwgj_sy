...
调用本技能后，智能体必须把每次任务纳入最小闭环：本次需求锁定 → JSON 变更 → 代码变更 → 验证证据 → 恢复点 → 变更记录 → 剩余风险。

执行顺序：判定任务 → 判定任务姿态 → 锁定用户本次需求 → 按需读取 JSON/切片 → 先更新 JSON → 再改代码 → 验证 → 更新恢复点和变更记录 → 汇报剩余风险。

冲突时按以下优先级裁决：用户明确不要 > 安全/数据/不可逆风险 > JSON 先行 > 本次需求锁定 > 功能簇主动性 > 执行速度。
...
任务姿态判定：开始任务时，先根据用户需求、是否存在 `architecture.json`、影响范围和风险信号，选择需要进入的姿态组合，如架构师、工程师、审计员、UI设计、恢复、风险模式。姿态只用于校准执行重点和必须校验项，不得接管需求判断，不得替代 JSON 先行，也不得变成多智能体调度。涉及模糊需求、新功能、UI、数据、安全、上下文恢复、测试失败或发布交付时，读取 `references/task-posture.md`；可运行脚本时，可用 `scripts/detect_task_posture.py --request "用户需求"` 输出姿态建议。
...
| 模糊需求、功能簇、功能树、反薄 Demo | `references/function-clusters.md` |

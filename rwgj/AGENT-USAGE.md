...
```text
1. skills/project-depth-core/SKILL.md
   先深度理解需求，展开功能簇，做架构归位和智能关联。

2. skills/architecture-json/SKILL.md
   把理解结果写入 `architecture/` 架构文件夹、模块详情、实现清单和验证责任。

3. skills/agent-protocol/SKILL.md
...
- 模糊需求、新功能、功能深度、交互闭环、反扁平化：先读 `skills/project-depth-core/SKILL.md`。
- 创建、修改、校验 `architecture/` 架构文件夹、`architecture/index.json` 或切片，或涉及模块、接口、数据、测试、恢复点：再读 `skills/architecture-json/SKILL.md`。
- 跨 Agent 使用、能力降级、虚拟模块审议、硬门禁、标准输出、动态姿势语境：按需读 `skills/agent-protocol/SKILL.md`。
...
4. 子技能中的 `../../shared/` 路径按同一规则解析：先解析为当前项目根目录的 `shared/`，不存在时再解析为全局技能安装目录的 `shared/`。

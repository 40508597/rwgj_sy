...
**`architecture/` 架构文件夹中的节点之间有天然的关联关系。修改一个节点时，AI 沿着关联链自动"联想"到受影响的其他节点——不需要死记硬背规则，而是理解节点之间的牵引关系。**

### 关联链模型

架构文件夹中的每个节点都属于某种类型，不同节点类型之间存在固定的关联牵引力：
...
修改 `architecture/index.json` 或相关切片时，分三步自动联想：

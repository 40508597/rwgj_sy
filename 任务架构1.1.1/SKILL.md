...
每次对用户汇报时，至少包含：
- 任务判断：创建 / 分析 / 追加 / 修改 / 校验
- 功能簇展开：核心功能、衍生功能、关联功能
- 影响范围：模块、接口、文件、数据、交互、测试
...
**必须中文：**
- JSON 对象键名：如 `项目信息`、`模块列表`、`接口契约`、`实现清单`、`完整细节`、`变更记录`
- 说明性字段：如 `说明`、`原因`、`影响范围`、`验证结果`、`剩余风险`
- 状态值和分类值：如 `计划中`、`编写中`、`待测试`、`已完成`、`基础设施`、`业务`
- 测试用例描述、交互说明、异常说明、变更记录正文

**保留原样：**
- 真实代码标识符：函数名、类名、变量名、模块名、包名
- 文件路径、路由路径、HTTP 方法、数据库表名和字段名
- 技术栈名、第三方库名、协议名和命令行参数

**禁止：**
- 新建 architecture.json 时使用英文主键名（如 `project`、`modules`、`contracts`、`changeLog`）
- 为了中文化而改写真实代码契约（如把 `validate_session` 写成 `验证会话`）
- 同一层级中中英文键名混用（如同时存在 `模块列表` 和 `modules`）
